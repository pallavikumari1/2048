    ...
    def init_grid(self):
        ...
        for i in range(c.GRID_LEN):
            ...
            for j in range(c.GRID_LEN):
                cells=Frame(background,bg=c.BACKGROUND_COLOR_CELL_EMPTY,width=c.SIZE/c.GRID_LEN,height=c.SIZE/c.GRID_LEN)
                cells.grid(row=i,column=j,padx=c.GRID_PADDING,pady=c.GRID_PADDING)
                t= Label(master=cells,text="",bg=c.BACKGROUND_COLOR_CELL_EMPTY,justify=CENTER,font=c.FONT,width=5,height=2)
                t.grid()
                grid_row.append(t)
    ...
    def key_down(self,event):
        ...
        if key in self.commands:
            ...
            if changed:
                ...
                self.update_grid_cells()
                changed=False
                if l.get_current_status(self.matrix)=='WON':
                    self.grid_cells[1][1].configure(text="You",bg=c.BACKGROUND_COLOR_CELL_EMPTY)
                    self.grid_cells[1][2].configure(text="Win!",bg=c.BACKGROUND_COLOR_CELL_EMPTY)
                elif l.get_current_status(self.matrix)=='LOST':
                    self.grid_cells[1][1].configure(text="You",bg=c.BACKGROUND_COLOR_CELL_EMPTY)
